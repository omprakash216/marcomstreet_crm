...
const DEFAULT_QUOTATION_TEMPLATE = 'standard';
...
// Match the logo's brand background instead of template accent colors when a logo exists.
const LOGO_BRAND_ACCENT = [44, 134, 171];
...
function getQuotationTemplate(key) {
...
function resolveLocalPath(candidate) {
  if (!candidate) return '';
  const cleaned = String(candidate).replace(/^file:\/\//i, '').replace(/^\/+/, '');
  if (path.isAbsolute(cleaned)) return cleaned;
  return path.join(REPO_ROOT, cleaned);
}

async function loadLogoDataUrl(rawSettings = {}) {
  const source = String(rawSettings.logo_url || rawSettings.logo_path || '').trim();
  if (!source) return '';
...
function drawHeader(doc, q, settings, config, logoDataUrl, meta) {
  ...
  if (config.header === 'left') {
    const headerTop = 14;
    const logoSize = 38;
    drawLogoSlot(doc, logoDataUrl, ml, headerTop, logoSize, logoSize, accent);
    // Keep the brand block in the same visual band as the logo so the header feels balanced.
    drawCompanyBlock(doc, settings, ml + logoSize + 8, headerTop + 6, 82, {
      nameColor: [17, 24, 39],
      nameSize: 11,
      fontSize: 6.6,
      lineHeight: 3.6,
...
function drawTotals(doc, q, config, startY, settings = {}) {
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(17, 24, 39);
  doc.text(toIndianRupeesWords(total), ml, startY + 5.5);

  const sigY = startY + 48;
  let bankEndY = startY + 15;
  let contentBottom = Math.max(y + 10, sigY + 24, startY + 48);
  ...
  if (settings.bank_name || settings.account_number) {
    ...
    contentBottom = Math.max(contentBottom, bankEndY);
  }

  const notes = String(q.notes || '').trim();
  const terms = String(q.terms_conditions || q.payment_terms || '').trim();
  if (notes || terms) {
    const sectionLineY = Math.max(bankEndY + 3.5, startY + 47);
    const sectionRight = x - 6;

    const lineGap = 4.0;
    const labelGap = 2;
    let infoY = sectionLineY + 5.5;

    const drawInlineInfo = (label, value, y) => {
      if (!value) return y;
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(7.8);
      doc.setTextColor(...accent);
      ...
      return y + Math.max(wrapped.length, 1) * lineGap;
    };
    ...
    contentBottom = Math.max(contentBottom, infoY);
  }
...
function toIndianRupeesWords(value) {
  ...
  if (numeric === 0) return 'Indian Rupee Zero Only';
  ...
  const hundred = Math.floor((numeric % 1000) / 100);
  ...
  return `Indian Rupee ${parts.join(' ')} Only`.replace(/\s+/g, ' ').trim();
}
...
async function generateDocumentPdf(documentData, rawSettings = {}, options = {}) {
  ...
  const hasBrandLogo = Boolean(String(settings.logo_url || settings.logo_path || '').trim());
  const config = {
    ...(TEMPLATE_CONFIG[template.key] || TEMPLATE_CONFIG.standard),
    accent: hasBrandLogo ? LOGO_BRAND_ACCENT : toRgb(TEMPLATE_CONFIG[template.key]?.accent || template.accent),
  };
  ...
  const logoDataUrl =
    template.key === 'standard' || template.key === 'minimal_clean'
      ? ''
      : await loadLogoDataUrl(settings);
  ...
  if (template.key !== 'minimal_clean') {
    ...
    for (let p = 1; p <= pages; p += 1) {
      ...
      // Keep the page border above the footer strip so the bottom edge stays visible.
...
async function generateQuotationPdfBuffer(quotation, rawSettings = {}) {
...
async function generateInvoicePdfBuffer(invoice, rawSettings = {}) {
