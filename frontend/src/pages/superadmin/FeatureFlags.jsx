...
export default function FeatureFlags() {
  ...
  const [form, setForm] = useState({ feature_name: '', status: 'disabled', description: '' });
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get('/superadmin/feature-flags');
  ...
  const createFlag = async (e) => {
    ...
    try {
      const res = await api.post('/superadmin/feature-flags', form);
      if (res.data?.success) {
        setForm({ feature_name: '', status: 'disabled', description: '' });
        await load();
      }
    } catch (err) {
  ...
  const toggle = async (row, status) => {
    try {
      const res = await api.patch(`/superadmin/feature-flags/${row.id}/toggle`, { status });
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-r from-white via-slate-50 to-white px-6 py-5 shadow-sm">
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Gradually roll out AI, automation and advanced reports across tenants.</p>
          </div>
        </div>
      </div>

      <form onSubmit={createFlag} className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            <label className="text-xs font-semibold text-slate-600">Feature name</label>
            <input
          ...
          <div>
            ...
            <input
              ...
              placeholder="Controls advanced report widgets"
              className="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 outline-none"
            />
          </div>
        </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td className="px-4 py-6 text-slate-500" colSpan={4}>No feature flags configured yet.</td></tr>
              ) : rows.map((f) => (
                <tr key={f.id} className="hover:bg-slate-50/60">
                  <td className="px-4 py-3 text-slate-900 font-semibold">{f.feature_name}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${f.status === 'enabled'
