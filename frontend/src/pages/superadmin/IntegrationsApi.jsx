...
export default function IntegrationsApi() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get('/superadmin/integrations/api-keys');
  ...
  const createKey = async () => {
    try {
      const res = await api.post('/superadmin/integrations/api-keys', {});
      if (res.data?.success) {
        await load();
        alert(`API key created: ${res.data.api_key}`);
      }
    } catch (err) {
  ...
  const revoke = async (id) => {
    try {
      const res = await api.patch(`/superadmin/integrations/api-keys/${id}/revoke`);
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-r from-white via-slate-50 to-white px-6 py-5 shadow-sm">
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Provision keys for integrations, webhooks, and public API clients.</p>
          </div>
          <button
            onClick={createKey}
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td className="px-4 py-6 text-slate-500" colSpan={5}>No API keys yet.</td></tr>
              ) : rows.map((k) => (
                <tr key={k.id} className="hover:bg-slate-50/60">
                  <td className="px-4 py-3 font-mono text-xs text-slate-800 break-all">{k.api_key}</td>
                  <td className="px-4 py-3 text-slate-700">{k.company_name || '-'}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${k.status === 'active'
